...
#
vmd -dispdev text -e tcl_scipts/extract_all_dcd_data.tcl >/dev/null

# Function to read in master config file: 
read_master_config_file() {
fpath=$1

if [ -f "$fpath"/master_config_file ]; then
...
# -- create a dcd file loader .vmd file in /OutputFiles 
 cd OutputFiles
  ./Tools/create_dcd_loader_script
 cd ../
...
if [[ -d $JOB_DIR ]]; then
  ...
  do
    ...
    cd $i
    index=$(echo $i | sed 's/.*_//')

    # grab directory name
    echo -e " Extracting dcd file details in directory $JOB_DIR/$i \n "
    ./tcl_scipts/create_dcd_loader_script

    # merge files: 
    cat OutputFiles/local_dcd_file_loader.vmd  >> ../../Analysis/${combined}_${index}.vmd
    cat OutputFiles/dcdfile_list.txt  >> ../../Analysis/dcdfile_list_${index}.txt
